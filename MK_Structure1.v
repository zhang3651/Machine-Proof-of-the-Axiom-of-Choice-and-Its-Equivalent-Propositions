...
Class MK_Axioms :Prop:= {
  ...
  A_VIII : ∃ y, Ensemble y /\ Φ ∈ y /\ (∀ x, x ∈ y -> (x ∪ [x]) ∈ y); }.
...
Parameter AxiomIX : ∃ c, ChoiceFunction c /\ dom(c) = μ ~ [Φ].

(* Lemma not_all_not_ex :
 ∀ P:Class -> Prop, ~ (∀ n:Class, ~ P n) -> ∃ n : Class, P n.
Proof.
  intros P notall. apply NNPP. intro abs. apply notall.
  intros n H. apply abs; exists n; exact H.
Qed.

Lemma not_all_ex_not :
 ∀ P:Class -> Prop, ~ (∀ n:Class, P n) -> ∃ n : Class, ~ P n.
Proof.
  intros P notall. apply not_all_not_ex with (P:=fun x => ~ P x).
  intro all; apply notall. intro n; apply NNPP. apply all.
Qed.

Lemma not_ex_all_not :
 ∀ P:Class -> Prop, ~ (∃ n:Class, P n) -> ∀ n : Class, ~ P n.
Proof.
  unfold not; intros P notex n abs. apply notex.
  exists n; trivial.
Qed. *)
