...
Proof.
  intros. unfold PartialOrder. split. apply MKT33 in H; auto. apply H1. 
  repeat split.
  ...
  - unfold Reflexivity; intros. unfold Rrelation. apply AxiomII. split.
    apply MKT49a; unfold Ensemble; eauto. unfold PartialOrder in H1.
    destruct H1 as [_[_[HR[HA HT]]]]. unfold Reflexivity in HR. 
    specialize (HR a). unfold Included in H. specialize (H a). New H2. 
    apply H in H2. apply HR in H2.
    unfold Rrelation in H2. split. apply H2. apply AxiomII. 
    split. unfold Ensemble; eauto. exists a. exists a. auto.
  - unfold Antisymmetry; intros. destruct H3. unfold Rrelation in H3, H4.
    ...
    specialize (HA x y). New H. unfold Included in H. specialize (H x). 
    destruct H2. 
    apply H in H2. unfold Included in H1. specialize (H1 y).  
    apply H1 in H3. apply HA. split; auto. split; auto.
  - unfold Transitivity; intros. New H. unfold Rrelation. apply AxiomII.
    split. apply MKT49a; destruct H2 as [[H8 [H4 H5]][H6 H7]]; 
    ...
    specialize (HT u v w). New H3; rename H3 into H2.
     unfold Included in H. specialize (H u).
    New H8; rename H8 into H6. apply H in H3. 
    unfold Included in H1. specialize (H1 v).
    New H4. apply H1 in H4. unfold Included in H2. specialize (H2 w).
    New H5. apply H2 in H5. split. apply HT; split ; auto. 
    apply AxiomII. split. apply MKT49a. unfold Ensemble; eauto. 
    unfold Ensemble; eauto. exists u. exists w.
    auto.
Qed.
    
Lemma Zorn'' : forall X le  y, PartialOrder le X -> y∈X -> y ∈ (F_x X le y).  
Proof.
  intros. apply AxiomII. split. unfold Ensemble; eauto. split. auto. 
  unfold PartialOrder in H. destruct H. destruct H1, H2. 
  ...
  - split; unfold Chain in H2; unfold F_x,FA_a, Included; intros.
    ...
    + New H. unfold Nest; intros. destruct H5. 
      ...
      * unfold Rrelation in H6. apply AxiomII in H6.
      rewrite H9. rewrite H11. unfold Included. left. intros. 
      ...
      * unfold Rrelation in H6. apply AxiomII in H6. rewrite H9. rewrite H11.
      unfold Included. right. intros. apply AxiomII. split. 
      unfold Ensemble; eauto. 
      apply AxiomII in H13. destruct H13 as [H16[H14 H15]]. split. auto.
      destruct H4. destruct H13. destruct H17 as [_[_HTR]].
  ...
  - unfold Chain. intros. split. split; auto. unfold TotalOrder. split.
    ...
    + intros. destruct H4. destruct H2. rename H7 into H8. rename H2 into H7.
      ...
      * apply AxiomII. destruct H. split. 
        apply MKT33 with X. auto. unfold Included. 
      ...
      * left. unfold Included in H9. specialize (H9 x). 
        ...
        auto. destruct H. destruct H10. destruct H11. auto. }
        apply H9 in H10. apply AxiomII in H10. destruct H10 as [_[_HR]].
        unfold Rrelation. apply AxiomII. split. apply MKT49a. 
        unfold Ensemble; eauto. unfold Ensemble; eauto.
        split. apply H10. apply AxiomII. split. apply MKT49a. 
        unfold Ensemble; eauto. unfold Ensemble; eauto. 
        exists x. exists y. auto.
      *  right. unfold Included in H9. specialize (H9 y). 
        assert (y ∈ (F_x X le y)).
        { apply AxiomII. split. unfold Ensemble; eauto. split. auto. 
        destruct H. destruct H10. destruct H11. auto. }
        apply H9 in H10. apply AxiomII in H10. destruct H10 as [_[_HR]].
        unfold Rrelation. apply AxiomII. split. apply MKT49a.
        unfold Ensemble; eauto. unfold Ensemble; eauto.
        split. apply H10. apply AxiomII. split. apply MKT49a.
...
Proof.
  ...
  - unfold Included; intros. apply AxiomII in H3 as [H5[a[H6 H7]]]. 
    ...
    split. unfold Ensemble; eauto. split. auto.  assert (Rrelation a le y).
    { unfold BoundU in H2. destruct H2. split. auto. intro. rewrite H2 in H9.
    feine z0; auto. destruct H3. specialize (H4 a). auto. }
    New H. destruct H. destruct H11. destruct H12, H13.
   ...
   - unfold BoundU. intros. split; auto. split. unfold Chain in H0.
    ...
    { apply AxiomII. split.  apply MKT33 with X. unfold PartialOrder in H.
...
Proof.
  ...
  - unfold MaxMember in H1. unfold MaxElement. intros. split; auto.
    intros. destruct H1. intro. feine (F_x X le y); auto.
    rewrite <- H1. apply AxiomII. split. apply MKT33 with X. 
    ...
    intro. apply H4. apply AxiomII. split. apply MKT33 with X. 
    ...
    unfold ProperIncluded. split. unfold Included. intros. apply AxiomII. split.
     unfold Ensemble; eauto. apply AxiomII in H7. destruct H7. 
     split. apply H8. destruct H8. New H. destruct H. destruct H11. 
     destruct H12, H13. unfold Transitivity in H14. apply H14 with (v:= y).
    repeat split; auto. intro.
    assert (∀z : Class,z ∈ (F_x X le y) <-> z ∈ (F_x X le y0)).
    { apply (AxiomI (F_x X le y) (F_x X le y0)); auto. } 
    clear H7; rename H8 into H7. specialize (H7 y0).
  ...
  - assert  (exists Fmax, MaxMember Fmax (FA_a X X le)).
    { apply MaxPrinciple. 
       ...
       { unfold Included; intros; unfold FA_a in H2.
        ...
        apply MKT38a in H; auto; apply MKT33 in H2; auto. 
      intros. 
      destruct H2. destruct (classic (n=Φ)).
      - apply NEexE in H1. destruct H1. exists (F_x X le x). split.
        apply AxiomII. split. apply MKT33 with X. apply H. unfold Included.
        intros. apply AxiomII in H5 as [_[HH_]]. auto. exists x; auto. intros.
        rewrite H4 in H5. apply MKT16 in H5. destruct H5.
      -  assert ((FA_a (En_A X n le) X le) ⊂ (FA_a X X le)).
        { intros; unfold Included; intros.
          ...
          New H9; apply AxiomII in H10; destruct H10, H11 as [a1 H11].
          destruct H11. New H12. New H12. 
          assert (∀z : Class,z ∈ (F_x X le a) <-> z ∈ (F_x X le a1)).
          { apply (AxiomI (F_x X le a) (F_x X le a1)); auto. } 
          clear H12; rename H15 into H12. specialize (H12 a).
          destruct H12. assert (a ∈ (F_x X le a)). apply Zorn''; auto. 
          apply H12 in H16.
          apply AxiomII in H16 as [_[_ HA]].
          assert (∀z : Class,z ∈ (F_x X le a) <-> z ∈ (F_x X le a1)).
          { apply (AxiomI (F_x X le a) (F_x X le a1)); auto. } 
          clear H13; rename H16 into H13.
          specialize (H13 a1).
          ...
          { split. unfold Included; intros. apply AxiomII in H7 as [_[HH_]]. 
            auto. apply NEexE in H4; destruct H4; New H4.
          apply H2 in H7; apply AxiomII in H7; destruct H7, H8, H8.
          rewrite H9 in H4; apply NEexE; exists x0.
          apply AxiomII; split; unfold Ensemble; eauto. } 
          assert (Chain (En_A X n le) X le).
          { apply Zornl; auto; apply H7. } New H8. apply H0 in H8. 
          destruct H8 as [y Ho]; unfold Chain in H9; New H.
          unfold PartialOrderSet in H8; apply H9 in H8; clear H9.
          destruct H8; exists (F_x X le y); split; intros.
          + unfold BoundU in Ho; New H.
            assert(PartialOrder le X /\ X ≠ Φ); auto.
            clear H10; rename H11 into H10. apply Ho in H10.
           ...
           destruct H; apply MKT33 in H11; auto.  
           exists y; split; auto.
          + assert (PartialOrder le X /\X <> Φ) as HH. split;auto.
            New H10; unfold Included in H2; apply H2 in H11.
            ...
            destruct HH. auto. apply AxiomII; split; auto; exists x; split; auto.
            apply AxiomII; split; unfold Ensemble; eauto; rewrite <- H13; auto. } 
      destruct H2 as [M H2]; New H2; unfold MaxMember in H3.
      assert (FA_a X X le ≠ Φ).
      { apply NEexE in H1; destruct H1.
        apply NEexE; exists (F_x X le x).
        unfold F_x; apply AxiomII; split.
        - destruct H; apply MKT33 with (x:= X); auto.
